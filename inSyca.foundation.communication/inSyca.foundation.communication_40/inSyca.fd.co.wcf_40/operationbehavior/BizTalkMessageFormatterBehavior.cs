...
{
    public class BizTalkMessageFormatterBehavior : IOperationBehavior
    {
        //Constructor.
        public BizTalkMessageFormatterBehavior()
        {
        }
        ...
        {
            Log.DebugFormat("ApplyDispatchBehavior(OperationDescription operationDescription {0}, DispatchOperation dispatchOperation {1}", operationDescription, dispatchOperation);

            //use our formater instead default(XML)
            dispatchOperation.Formatter = new BizTalkMessageFormatter(dispatchOperation.Formatter);
        }
