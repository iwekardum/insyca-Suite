...
using inSyca.foundation.integration.biztalk.adapter.diagnostics;
...
namespace inSyca.foundation.integration.biztalk.adapter.file.runtime
{
    ...
    /// </summary>
    internal class ReceiverEndpoint : common.ReceiverEndpoint
    {
        // constants
            ...
            ControlledTermination control)
        {
            ...
            this.properties = new ReceiveProperties();
        ...
        private bool CheckMinFileSize(FileInfo item)
        {
            if (item.Length > 0)
                return true;
            else
        ...
        private void PickupFilesAndSubmit ()
        {
            ...
            {
                //  only consider files that are greater 0
                if (false == CheckMinFileSize(item))
                    continue;
            ...
            internal void OnBatchComplete(bool overallStatus)
            {
                ...
                if (overallStatus == true) //Batch completed
                {
                    ...
                    foreach (BatchMessage batchMessage in messages)
                    {
                        ...
                        DeleteIntermediateFile(batchMessage, 0);
                    }
                }
            }

            /// <summary>
            /// Delete the intermediate file and try 10 times in case the file is still locked
            /// we delete the files from the folder
            /// </summary>
            /// <param name="batchMessage"></param>
            /// <param name="counter"></param>
            private static void DeleteIntermediateFile(BatchMessage batchMessage, int counter)
            {
                Log.DebugFormat("DeleteIntermediateFile(BatchMessage batchMessage {0}, int counter {1})", batchMessage, counter);
                ...
                catch (System.Exception)
                {
                    ...
                    Thread.Sleep(500);
                    counter++;
                    DeleteIntermediateFile(batchMessage, counter++);
                }
            }
        }
